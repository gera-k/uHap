...
namespace Hap::Http
{
	...
	{
		HTTP_200,	// when the request was successfully executed and the response includes a body, e.g. after successfully reading characteristics.
		HTTP_204,	// when the request was successfully executed and the response has no body, e.g. after successfully writing characteristics.
		HTTP_207,	// when the request was processed but was not executed (partially or completely), e.g. when writing two characteristics, one fails or both writes fails, when writing one characteristicand the write fails.
		HTTP_400,	// on HTTP client error, e.g. a malformed request.
		HTTP_404,	// on an invalid HTTP URL.
		//HTTP_405,
		HTTP_422,	// for a well-formed request that contains invalid HTTP parameters.
		//HTTP_429,
		HTTP_470,
		HTTP_500,	// on an accessory server error, e.g.the operation timed out.
		HTTP_503	// if the accessory server is too busy to service the request, e.g. reached its	maximum number of connections.
	};
	...
	public:
		...
		auto method()
		{
			return Buf(_method, _method_len);
		}

		auto path()
		{
			return Buf(_path, _path_len);
		}

		auto data()
		{
			return Buf(_data, _data_len);
		}
		...
		{
			if (i < _num_headers)
				return Buf(_headers[i].name, _headers[i].name_len);
			return Buf((const char*&)_buf, uint32_t(0));
		}

		auto hdr_value(uint32_t i)
		{
			if (i < _num_headers)
				return Buf(_headers[i].value, _headers[i].value_len);
			return Buf((const char*&)_buf, uint32_t(0));
		}
	...
	public:
		...
		//	assumes that _len_pos was saved by prevous call to add(ContentLength,0)
		void setContentLength(uint16_t len)
		{
			...
			snprintf(_buf + _len_pos - 4, 6, "%4d", len);
			_buf[_len_pos] = t;
	...
	private:
		...
		public:
			...
			uint64_t recvSeq;
			uint64_t sendSeq;
			Hap::Json::ParserStatic<200> wrParser;	// json parser for PUT, allocates storage for 200 tokens TODO: some other way
