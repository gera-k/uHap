...
namespace CryptoTest
{
	int ed25519_test()
	{
		struct _test
		{
			const uint8_t seed[Crypto::Ed25519::SEED_SIZE_BYTES];
			const uint8_t pubKey[Crypto::Ed25519::PUBKEY_SIZE_BYTES];
			const uint8_t msg[1024];
			const uint32_t msg_len;
			const uint8_t sign[Crypto::Ed25519::SIGN_SIZE_BYTES];
		} static test_list[] =
		{
			{
				{
		...
		HAP_PRINTF("ED25519 test");

		for (unsigned test = 0; test < sizeofarr(test_list); test++)
		{
			struct _test *t = &test_list[test];
			uint8_t sign[Crypto::Ed25519::SIGN_SIZE_BYTES];

			Crypto::Ed25519 ed;
			ed.init(t->seed);
			ed.sign(sign, t->msg, t->msg_len);

			if (memcmp(sign, t->sign, Crypto::Ed25519::SIGN_SIZE_BYTES) != 0)
			{
				HAP_PRINTF("Test %d: sign failure", test);
				r++;
			}

			if (!ed.verify(sign, t->msg, t->msg_len, t->pubKey))
			{
				HAP_PRINTF("Test %d: verify failure", test);
					r++;
			}
		}
