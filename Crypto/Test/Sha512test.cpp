...
namespace CryptoTest
{
	...
	int sha512_test()
	{
		...
		for (unsigned t = 0; t < sizeofarr(sha512_tst); t++)
		{
			...
			HAP_PRINTF("Test %d: '%s'", t, sha512_tst[t].m);
#if 0
			...
			{	// do whole message at once
				...
				for (int i = 0; i < 8; i++)
				{
					...
					{
						HAP_PRINTF("a. Word %d error: %016llX  %016llX", i, sha.h[i], sha512_tst[t].v[i]);
						r++;
					}
				}
			}
			...
			{	// do it in 100 byte increments
				...
				for (int i = 0; i < 8; i++)
				{
					if (sha.h[i] != sha512_tst[t].v[i])
					{
						HAP_PRINTF("b. Word %d error: %016llX  %016llX", i, sha.h[i], sha512_tst[t].v[i]);
						r++;
					}
				}
			}
