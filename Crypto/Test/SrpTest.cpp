...
namespace CryptoTest
{
	...
	int srp_test()
	{
		...
		{
			HAP_PRINTF("Verifier mismatch");
			r++;
		}
		d2 = Timer::now();
		HAP_PRINTF("SRP 1 (verifier init) duration: " TIMER_FORMAT "", Timer::ms(d1, d2));
		...
		{
			HAP_PRINTF("User Public value A mismatch");
			r++;
		}
		d2 = Timer::now();
		HAP_PRINTF("SRP 2 (user init) duration: " TIMER_FORMAT "", Timer::ms(d1, d2));
		...
		{
			HAP_PRINTF("Host Public value B mismatch");
			r++;
		}
		d2 = Timer::now();
		HAP_PRINTF("SRP 3.1 (host init) duration: " TIMER_FORMAT "", Timer::ms(d1, d2));

		d1 = Timer::now();
		host.setA(A);
		if (memcmp(K, host.getK(), sizeof(K)) != 0)
		{
			HAP_PRINTF("Host Session key K mismatch");
			r++;
		}
		d2 = Timer::now();
		HAP_PRINTF("SRP 3.2 (host setA) duration: " TIMER_FORMAT "", Timer::ms(d1, d2));
		...
		{
			HAP_PRINTF("User Session key K mismatch");
			r++;
		}
		d2 = Timer::now();
		HAP_PRINTF("SRP 4 (user auth) duration: " TIMER_FORMAT "", Timer::ms(d1, d2));
		...
		user.proof(M);
		d2 = Timer::now();
		HAP_PRINTF("SRP 5 (user proof) duration: " TIMER_FORMAT "", Timer::ms(d1, d2));
		...
		{
			HAP_PRINTF("User->Host M verifiation failed");
			r++;
		}

		if (!user.verify(V, sizeof(V)))
		{
			HAP_PRINTF("Host->User V verifiation failed");
			r++;
		}
		d2 = Timer::now();
		HAP_PRINTF("SRP 6 (user/host verify) duration: " TIMER_FORMAT "", Timer::ms(d1, d2));
